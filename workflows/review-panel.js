export const meta = {
  name: "review-panel",
  description: "Deep final review of a completed change: a fan-out of reviewers (each on a distinct lens) closed by a ≥Opus integrator that merges them into ONE verdict (most severe wins). The coordinator chooses the composition via reviewModels — a single Opus/Fable reviewer, an all-Opus/all-Fable/mixed panel, or the two-tier pattern (an Opus panel then a Fable final verdict) — or omits it and a cheap Sonnet composer picks. Mirrors design-panel; called for the whole-change gate after per-wave verification.",
